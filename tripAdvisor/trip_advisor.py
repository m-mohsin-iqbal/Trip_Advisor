...
class TripAdvisor():
    ...
    def parse(self):
        ...
        for q1,q2,category in zip(df['q1'],df['q2'],df["category"]):
            if "things" in category.lower():
                ...
                if driver.find_elements_by_css_selector(".result-title"):
                    for i in range(len(driver.find_elements_by_css_selector(".result-title"))):
                        ...
                        try:
                            item["rank"] = driver.find_element_by_css_selector('[data-test-target="restaurant-detail-info"] div span+span a span').text
                            if not item['rank']:
